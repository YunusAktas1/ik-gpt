...
function AgentPipe({steps,cur}){const M={cv:"📄 CV Analizi",match:"🎯 Eşleştirme",questions:"💬 Soru Üretimi",notify:"✉️ Bildirim"};return(<div style={{background:"var(--sand2)",border:"1px solid var(--line)",borderRadius:"var(--r)",padding:14,marginTop:10}}><div style={{fontSize:10,fontFamily:"var(--mono)",color:"var(--muted)",marginBottom:8}}>⚡ AI İŞLEM HATTI</div>{steps.map((k,i)=>{const done=i<cur;const act=i===cur;return(<div key={k} style={{display:"flex",alignItems:"center",gap:10,padding:"4px 0",opacity:i>cur?.4:1}}><div style={{width:20,height:20,borderRadius:"50%",background:done?"var(--green2)":act?"var(--blue3)":"var(--sand3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,flexShrink:0}}>{done?"✓":act?<Spin s={8}/>:i+1}</div><div style={{fontSize:12,fontWeight:600,color:done?"var(--green)":act?"var(--blue)":"var(--muted)"}}>{M[k]||k}{act&&<span style={{fontSize:10,color:"var(--muted)",marginLeft:6,animation:"pulse 1.5s infinite"}}>işleniyor...</span>}</div></div>)})}</div>)}
